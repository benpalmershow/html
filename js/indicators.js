...
function buildChangeMetricButton(label, changeInfo, title) {
    ...
    return `<div class="change-metric-block"><button type="button" class="change-metric-btn ${changeInfo.cssClass}" title="${title.replace(/"/g, '&quot;')}" aria-label="${label ? label + ' ' : ''}${changeInfo.formatted}">${topSection}<span class="change-metric-value">${valueWithIcon}</span></button></div>`;
}
